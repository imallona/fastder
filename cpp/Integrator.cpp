...
{
    // Single-pass chain-building per chromosome, ignoring SJ strand. This
    // matches upstream's behaviour exactly so the perf branch's output GTF
    // is identical to upstream's on the same input. Strand-aware stitching
    // (bucketing SJs by SJRow.strand and running two passes per chromosome)
    // is a separate, output-changing feature that should be added in its own
    // branch and validated against simulated truth in fastder-evaluation.
    // The strand fields on BedGraphRow / SJRow / StitchedER stay in place so
    // that follow-up work has the plumbing ready.
    for (auto& sjs : mm_chrom_sj)
    {
        std::string chrom = sjs.first;
        StitchedER er1 = StitchedER(expressed_regions.at(chrom).at(0), 0); // first ER becomes the first stitched ER
        stitched_ERs.emplace_back(er1);
        auto current_sj_id = sjs.second.begin();

        int max_stitched_ers = 0;
        int nof_stitched_ers = 0;
        for (int i = 1; i < static_cast<int>(expressed_regions.at(chrom).size()); ++i)
        {
            const auto& expressed_region = expressed_regions[chrom][i];
            if (current_sj_id != sjs.second.end())
            {
                StitchedER& current_stitched_er = stitched_ERs.back();

                // skip ahead to a SJ whose start lines up with the chain's end
                while (current_sj_id != sjs.second.end()
                    && (current_stitched_er.end > rr_all_sj[*current_sj_id - 1].start
                        && !within_threshold(current_stitched_er.end, rr_all_sj[*current_sj_id - 1].start))
                ...
                {
                    uint64_t sj_length = expressed_region.start - expressed_regions[chrom][current_stitched_er.er_ids.back()].end;
                    current_stitched_er.append(-1, sj_length, 0.0); // append the intron placeholder
            ...
            {
                // no more SJs on this chromosome: each remaining ER forms its own StitchedER
                stitched_ERs.emplace_back(StitchedER(expressed_region, i));
            }
        }
